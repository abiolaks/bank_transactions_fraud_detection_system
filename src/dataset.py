...
def generate_synthetic_transactions(
    num_customers=100, start_date="2024-01-01", end_date="2024-03-31", seed=42
):
    random.seed(seed)
    np.random.seed(seed)
    ...
    df = (
        ...
        .sort_values(by="transaction_time")
        .reset_index(drop=True)
    )
    df.to_csv("../data/raw/base_bank_transactions.csv", index=False)
